...
Previously, we discussed how `<Canvas />` creates a droppable Canvas node; that concept is applied universally in PageZilla. Hence, we just have to wrap our `section` elements in the example above with a `Canvas`. 
